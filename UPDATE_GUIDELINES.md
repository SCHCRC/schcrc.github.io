...
3. **한 가지를 한 번만 말한다.** 제목이 이미 설명한 것을 리드 문단이 되풀이하면 리드를 지웁니다.
   - 앞 문장이 이미 한 일을 말했으면 "~하는 것이 목표였습니다 / 목적입니다 / 범위입니다" 를 뒤에 붙이지 않습니다. 프로젝트 설명 9개 중 5개가 그 꼴이었습니다.
   - 화면에 이미 있는 것을 글로 또 안내하지 않습니다. 첨부 블록 바로 위의 "하단 첨부파일에서 다운로드할 수 있습니다" 같은 문장입니다.
   - **사이트가 자기 사정을 설명하지 않습니다.** "발표자료에 적힌 것만 옮겼습니다", "이전 홈페이지에서 이관했습니다" 는 방문자가 알 필요 없는 운영 기록입니다.
   - "순차적으로 / 체계적으로 / 효율적으로", "같은 출발선" 같은 상투어는 쓰지 않습니다. 무엇을 했는지만 적으면 됩니다.
4. **정보 원본은 한 곳에만 둔다.** 프로젝트는 `_project_pages/*.md` 하나가 원본입니다. 같은 내용을 데이터 파일에 또 적지 않습니다.
   - 수상·선발 내역은 `_data/achievements.yml` 하나가 원본입니다. 연구원 페이지의 "주요 성과"는 여기서 slug 로 걸러 그립니다. `_people` 에 `achievements` 를 적으면 검사기가 막습니다 — 두 곳에 두었을 때 같은 사건의 표기가 6건 어긋났습니다("AhnLab ASEC 분석팀 진출" vs "ASEC 분석팀 합격" 등).
5. **올리기 전에 `tools/verify.py` 를 돌린다.** 통과하지 않으면 배포하지 않습니다.
6. **센터에 관한 사실은 학기 발표자료를 기준으로 삼는다.** 바로 아래 1-1 절 참조.

---

## 1-1. 사실 확인의 기준

센터장 이름, 소속, 설립 배경, 연구 분야, 연구원 실적처럼 **센터에 관한 사실**은
매 학기 만드는 센터 소개 발표자료를 기준으로 삼습니다. 사이트 문구가 자료와
어긋나면 자료가 맞습니다.

기준 자료: `26년도 1학기 SCH사이버보안연구센터 발표자료` (2026-08 대조 완료)

| 항목 | 자료 위치 | 사이트 위치 |
|---|---|---|
| 센터장·소속·위치·설립 배경 | 3쪽 | `index.md` 의 `hero__facts`, `PRODUCT.md` |
| 연구 분야 3종 | 4쪽 | `_data/site.yml` |
...
자료 3쪽은 "순천향대학교 정보보호학과 염흥열 교수님"입니다. 사람 이름과
기관 이름은 검사기가 잡아줄 수 없으니 자료와 글자 단위로 맞춰 보세요.

**센터장 직함은 "교수"입니다.** 자료 7쪽에 실린 2024년 보안뉴스 기사에는
"염흥열 명예교수"로 적혀 있어 헷갈릴 수 있는데, 자료 본문(3쪽) 표기가
기준이고 센터에서 "교수"로 확인했습니다. 기사 쪽을 보고 "명예교수"로
고치지 마세요.

**자료에 없는 것은 쓰지 않습니다.** 예를 들어 자료가 수상 내역만 적고 소속
